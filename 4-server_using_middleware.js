...
//     AUTH MIDDLEWARE    |
//=========================
//app.use - will use the block of code everytime a route is being called
//  in our case - will use our auth middleware in every route
//next() - will continue the route normally

app.use((req, res, next) => {
  console.log("We are using middleware!!!!!")
  if(!(req.session && req.session.userId)) {
    console.log("User without session");
    return next();
  }

  User.findById(req.session.userId, (err, user) => {
    if(err) {
      return next(err); //continue with error
    }

    if(!user) {
      console.log("Email or password aren't matched");
      return res.redirect("/login");
    }

    user.password = undefined;
    res.locals.user = user; //will render the user data to tamplates
